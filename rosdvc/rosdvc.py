...
from rosdvc.CaptioningHead import build_captioner
import copy
...
def torch_isin(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    out = (a_flat[:, None] == b_flat[None, :]).any(dim=1)
...
def CTCA(matched_loc_query, matched_cap_query,
                     unmatched_loc_query, unmatched_cap_query, temperature=0.07):

    matched_loc_query = F.normalize(matched_loc_query, dim=-1)
    matched_cap_query = F.normalize(matched_cap_query, dim=-1)
    unmatched_loc_query = F.normalize(unmatched_loc_query, dim=-1)
    unmatched_cap_query = F.normalize(unmatched_cap_query, dim=-1)

    pos_sim = (matched_loc_query * matched_cap_query).sum(-1)  # [M]

    neg_sim1 = matched_loc_query @ unmatched_cap_query.T  # [M, U]

    neg_sim2 = matched_cap_query @ unmatched_loc_query.T  # [M, U]

    pos_sim_scaled = pos_sim / temperature
    neg_sim1_scaled = neg_sim1 / temperature
    neg_sim2_scaled = neg_sim2 / temperature

    logits1 = torch.cat([pos_sim_scaled.unsqueeze(1), neg_sim1_scaled], dim=1)  # [M, 1+U]
    labels1 = torch.zeros(matched_loc_query.size(0), dtype=torch.long, device=logits1.device)
    loss1 = F.cross_entropy(logits1, labels1)

    logits2 = torch.cat([pos_sim_scaled.unsqueeze(1), neg_sim2_scaled], dim=1)  # [M, 1+U]
    labels2 = torch.zeros(matched_cap_query.size(0), dtype=torch.long, device=logits2.device)
    loss2 = F.cross_entropy(logits2, labels2)

    return loss1

class ROS_DVC(nn.Module):
    """ This is the ROS-DVC module that performs dense video captioning """
    ...
    def __init__(self, base_encoder, transformer, captioner, num_classes, num_queries, num_feature_levels,
        ...
        Parameters:
            ...
            num_queries: number of event queries. This is the maximal number of events
                         ROS-DVC can detect in a single video. For ActivityNet Captions, we recommend 10-30 queries.
        ...
        self.class_head = nn.Linear(hidden_dim, num_classes)                     ## localization head
        self.count_head = nn.Linear(hidden_dim, opt.max_eseq_length + 1)         ## event counter
        self.bbox_head = MLP(hidden_dim, hidden_dim, 2, 3)                       ## localization head

        self.concept_guider = nn.Sequential(
                                nn.Linear(hidden_dim, hidden_dim*2),
                                nn.ReLU(),
                                nn.Linear(hidden_dim*2, opt.concept_num)
                            )                 ## concept head                    ## concept Gudier
    ...
    def forward(self, dt, criterion, transformer_input_type, memory_bank_clip_gt, eval_mode=False):

        global clip_gt
        clip_gt = memory_bank_clip_gt
        ...
        if two_stage:
            ...
            query_embed_cap=self.query_embed_cap.weight
        ...
        hs_cap, inter_references_cap = self.transformer.forward_decoder(tgt_cap, reference_points, memory, temporal_shapes,
                                                                     level_start_index, valid_ratios, query_embed_cap,
                                                                     mask_flatten, proposals_mask_cap, disable_iterative_refine)
        ...
        if eval_mode or self.opt.caption_loss_coef == 0:  ##eval 이거나 , localization 성능만 볼때
            out, loss = self.parallel_prediction_full(dt, criterion, hs, hs_cap, init_reference, inter_references, others,
                                                      disable_iterative_refine)
        else:
            out, loss = self.parallel_prediction_matched(dt, criterion, hs, hs_cap, init_reference, inter_references, others,
                                                         disable_iterative_refine)
        return out, loss
    ...
    def parallel_prediction_full(self, dt, criterion, hs, hs_cap, init_reference, inter_references, others,
                                 disable_iterative_refine):
        outputs_classes = []
        outputs_classes0 = []
        outputs_coords = []
    ...
    def parallel_prediction_matched(self, dt, criterion, hs, hs_cap, init_reference, inter_references, others,
                                    disable_iterative_refine):
        outputs_classes = []
        outputs_counts = []
        outputs_coords = []
        ...
        for l_id in range(num_pred):
            ...
            seq_idxs = torch.arange(0, timestamp_len).repeat(query_len)

            with torch.no_grad():
                cost_caption, loss_caption, cap_probs, seq = self.caption_prediction_no_grad(self.caption_head[l_id], dt, hs_lid_cap,
                                                                                 reference, others, self.opt.caption_decoder_type,
                                                                                 [(query_idxs,seq_idxs)])
                num_queries = hs.shape[2]
                num_gt = len(dt["gt_timestamp"][0])
                cost_caption = cost_caption.view(num_queries, num_gt)
                loss_caption = loss_caption.view(num_queries, num_gt)
        ...
        if self.aux_loss:
            ks, vs = list(zip(*(all_out.items())))
            out['aux_outputs'] = [{ks[i]: vs[i][j] for i in range(len(ks))} for j in range(num_pred - 1)]
            loss, last_indices, aux_indices = criterion(out, dt['video_target'])  ## 여기서 matching이 이루어지겠네
            ...
            for l_id in range(hs.shape[0]):
                ...
                cap_loss, cap_probs, seq, concept_loss = self.caption_prediction(self.caption_head[l_id], dt, hs_lid_cap, reference,
                                                                   others, self.opt.caption_decoder_type, indices)

                l_dict = {'loss_caption': cap_loss, "concept_loss":concept_loss}
        ...
        last_hs = hs[-1]
        last_hs_cap = hs_cap[-1]
        loc_query = last_hs.squeeze(0)
        cap_query = last_hs_cap.squeeze(0)

        matched_query_indices = indices[0][0]

        matched_loc_query = loc_query[matched_query_indices]
        matched_cap_query = cap_query[matched_query_indices]

        unmatched_query_indices = torch.arange(loc_query.size(0))
        unmatched_query_indices = unmatched_query_indices[
            ~torch_isin(unmatched_query_indices, matched_query_indices)
        ]

        unmatched_loc_query = loc_query[unmatched_query_indices]
        unmatched_cap_query = cap_query[unmatched_query_indices]

        loss_CTCA = CTCA(
            matched_loc_query, matched_cap_query.detach(),
            unmatched_loc_query, unmatched_cap_query.detach(),
            temperature=self.opt.ctca_loss_temperature
        )
        
        loss.update({'loss_CTCA': loss_CTCA})
    ...
    def caption_prediction_no_grad(self, cap_head, dt, hs, reference, others, captioner_type, indices=None):
        ...
        if captioner_type == 'none':
            ...
            if self.training:
                ...
                cap_prob, _=cap_head(hs[:, feat_bigids], reference[:,feat_bigids], others, seq)
                cap_loss = cap_head.build_loss(cap_prob, caption_tensor[:, 1:], caption_mask)
                cap_cost = cap_loss
                ...
                with torch.no_grad():
                    ...
                    cap_cost = torch.tensor(0.0, device=hs.device)
                    cap_loss = torch.tensor(0.0, device=hs.device)
    ...
    def caption_prediction(self, cap_head, dt, hs, reference, others, captioner_type, indices=None):
        ...
        if indices == None: 
            ...
            feat_bigids = torch.zeros(sum([len(_[0]) for _ in indices])).long()  ## [0,0,0,0]
            cap_bigids = torch.zeros_like(feat_bigids)
            total_query_ids = 0
            total_cap_ids = 0
            total_ids = 0
            ...
            for i, index in enumerate(indices):
                feat_ids, cap_ids = index
                feat_bigids[total_ids: total_ids + len(feat_ids)] = total_query_ids + feat_ids  ##[q1,q2,q2,q4 ..]
                cap_bigids[total_ids: total_ids + len(feat_ids)] = total_cap_ids + cap_ids
                new_hr_for_dsa[i, :len(feat_ids)] = hs[i, feat_ids]
                new_seq_for_dsa[i, :len(feat_ids)] = cap_seq[total_cap_ids + cap_ids]
                total_query_ids += query_nums[i]
        ...
        hs_sel = hs_r[feat_bigids]                               # [num_matched, C]

        concept_labels = dt['concept_labels'][cap_bigids]        # [num_matched, Nc]
        concept_labels = concept_labels.to(device).float()

        concept_logits = self.concept_guider(hs_sel)               # [num_matched, Nc]

        assert concept_logits.shape == concept_labels.shape, \
            f"concept logits {concept_logits.shape} vs labels {concept_labels.shape}"

        criterion_concept = nn.BCEWithLogitsLoss()
        concept_loss = criterion_concept(concept_logits, concept_labels)
        ...
        if captioner_type == 'none':
            ...
            if self.training:
                ...
                if self.opt.caption_cost_type != 'rl':
                    cap_prob, state = cap_head(hs[:, feat_bigids], reference[:, feat_bigids], others, seq)
                    cap_probs['cap_prob_train'] = cap_prob
            else:
                with torch.no_grad():
                    cap_prob, state = cap_head(hs[:, feat_bigids], reference[:, feat_bigids], others,
        ...
        if indices:
            return cap_loss.mean(), cap_probs, seq, concept_loss
        ...
        cost_caption = cost_caption.reshape(-1, N_q,
                                            max(cap_id) + 1)  # batch_size * num_queries * all_caption_num
        loss_caption = loss_caption.reshape(-1, N_q, max(cap_id) + 1)
        return cost_caption, loss_caption, cap_probs, seq, concept_loss
...
def build(args):
    ...
    weight_dict = {'loss_ce': args.cls_loss_coef,
                   'loss_OSL': args.osl_coef,
                   'loss_bbox': args.bbox_loss_coef,
                   'loss_giou': args.giou_loss_coef,
                   'loss_counter': args.count_loss_coef,
                   'loss_caption': args.caption_loss_coef,
                   'concept_loss': args.concept_loss_coef,
                   'loss_CTCA': args.ctca_loss_coef,
                   }
