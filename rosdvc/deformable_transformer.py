...
from rosdvc.ops.modules import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    ...
    def prepare_encoder_inputs(self, srcs, masks, pos_embeds):
        ...
        src_flatten = torch.cat(src_flatten, 1)  # (b,lvl_num,c)
        mask_flatten = torch.cat(mask_flatten, 1)  # (b,lvl)
        lvl_pos_embed_flatten = torch.cat(lvl_pos_embed_flatten, 1)  # (b,lvl_num_c)
        temporal_shapes = torch.as_tensor(temporal_shapes, dtype=torch.long, device=src_flatten.device)  # 총 lvl 개수 여기선 4 
        level_start_index = torch.cat((temporal_shapes.new_zeros((1,)), temporal_shapes.cumsum(0)[
                                                                       :-1]))  # prod: [w0h0, w0h0+w1h1, w0h0+w1h1+w2h2, ...]  ## [0,157,236,279] 각 레벨별로 나타나는 L 사이즈를 저장한 것
