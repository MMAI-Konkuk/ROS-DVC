...
class PositionEmbeddingSine(nn.Module):
    ...
    def duration_embedding(self, durations):
        d = durations
        if d.dim() > 1:
            d = d.view(d.size(0))                                         # [B]로 평탄화  #추가1
        d = d.to(torch.float32)                                           #추가2
        d = torch.ceil(d)                                                 #추가3  # 내림 대신 천장
        d = torch.clamp(d, min=1.0, max=float(self.max_duration))         #추가4  # 최소 1 보장
        d = d.to(torch.long) 

        ar = torch.arange(self.max_duration, device=durations.device)     # [Dmax]
        out = (ar.unsqueeze(0) < d.unsqueeze(1)).to(torch.float32)
