...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        sampling_offsets = self.sampling_offsets(query).view(N, Len_q, self.n_heads, self.n_levels, self.n_points)  ## 각 n_level에 대해 m_point개 좌표
        attention_weights = self.attention_weights(query).view(N, Len_q, self.n_heads, self.n_levels * self.n_points) ## 모든 n_level에 대해 n*m개의 좌표에대해 
        attention_weights = F.softmax(attention_weights, -1).view(N, Len_q, self.n_heads, self.n_levels, self.n_points) ## n*m에 해당하는 좌표에 대해 softmax를 걸어 attention_weight 구함
        ...
        if True:  ## 기존의 detr은 2d 이미지에서 처리 하기 때문에, dvc에서는 time축(1d)만 사용하기에 억지로 2d로 맞춤 (time,0,5)
