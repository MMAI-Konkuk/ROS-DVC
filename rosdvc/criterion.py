...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_OSL = OSL(target_boxes, outputs)
        losses['loss_OSL'] = loss_OSL
...
def pairwise_iou_matrix(boxes, eps = 1e-7):
    B, Q, D = boxes.shape
    if D == 2:
        # [center, length] -> [start, end]
        c, l = boxes.unbind(-1)                  # [B,Q], [B,Q]
        l = l.clamp(min=0)                       # 길이 음수 방지 (안전장치)
        s = (c - 0.5 * l).unsqueeze(-1)          # [B,Q,1]
        e = (c + 0.5 * l).unsqueeze(-1)          # [B,Q,1]

        # pairwise IoU
        sT = s.transpose(-2, -1)                 # [B,1,Q]
        eT = e.transpose(-2, -1)
        inter = (torch.minimum(e, eT) - torch.maximum(s, sT)).clamp(min=0)
        len_i = (e - s).clamp(min=0)
        len_j = (eT - sT).clamp(min=0)
        union = len_i + len_j - inter + eps
        return inter / union                     # [B,Q,Q]

    else:
        raise ValueError(f"Unsupported box dim D={D}, expected 2 or 4.")
    
def GT_pred_iou_matrix(boxes, targets, eps = 1e-7):
    B, Q, D = boxes.shape
    _, T, D2 = targets.shape

    assert D == D2 == 2, f"Unsupported box dim D={D}, expected 2."

    c, l = boxes.unbind(-1)          
    l = l.clamp(min=0)
    s = (c - 0.5 * l).unsqueeze(-1)  
    e = (c + 0.5 * l).unsqueeze(-1) 

    ct, lt = targets.unbind(-1)      
    lt = lt.clamp(min=0)
    st = (ct - 0.5 * lt).unsqueeze(1) 
    et = (ct + 0.5 * lt).unsqueeze(1) 
...
def OSL(targets, outputs, opt=None,  gamma = 0.35, beta = 1):
    if opt is not None:
        beta = getattr(opt, 'osl_beta', beta)
    boxes = outputs['pred_boxes']
    targets = targets.unsqueeze(0) 
    iou = pairwise_iou_matrix(boxes) 
    off_diag = iou - torch.diag_embed(iou.diagonal(dim1=-2, dim2=-1))
    mask = off_diag != 0
    density_norm = (off_diag * mask).sum(-1) / mask.sum(-1).clamp(min=1)

    giou = GT_pred_iou_matrix(boxes, targets)
    mask = giou != 0
    gdensity_norm = (giou * mask).sum(-1) / mask.sum(-1).clamp(min=1)
    
    safe_term = (beta - density_norm).clamp(min=1e-6)
    alpha = gamma * gdensity_norm + (1 - gamma) * (1 - gdensity_norm) # 18
    loss = alpha * (-torch.log(safe_term))
