...
rosdvc_dir = dirname(abspath(__file__))
sys.path.insert(0, rosdvc_dir)
sys.path.insert(0, os.path.join(rosdvc_dir, 'densevid_eval3'))
sys.path.insert(0, os.path.join(rosdvc_dir, 'densevid_eval3/SODA'))
# print(sys.path)
...
from rosdvc.rosdvc import build
from collections import OrderedDict
from ret_utils import load_clip_gt
...
def train(opt):
    ...
    with open(opt.concept_stoi_file, "r") as f:
    ...
    while True:
        ...
        for dt in tqdm(train_loader, disable=opt.disable_tqdm):
            ...
            output, loss = model(dt, criterion, opt.transformer_input_type, memory_bank_clip_gt)
        ...
        if (epoch % opt.save_checkpoint_every == 0) and (epoch >= opt.min_epoch_when_save):
            ...
            eval_score, eval_loss = evaluate(model, memory_bank_clip_gt, criterion, postprocessors, val_loader, result_json_path, logger=logger, alpha=opt.ec_alpha, device=opt.device, debug=opt.debug)
