...
def get_topn_from_dvcjson(dvc_json, out_json, top_n=3, 
                          ing_key='proposal_score', score_thres=-1e8):
    data = json.load(open(dvc_json, 'r'))['results']
    out = {}
    out['version'] = "VERSION 1.0"
    out['external_data'] = {'used:': True, 'details': "GT proposals"}
...
def evaluate(model, memory_bank, criterion, postprocessors, loader, dvc_json_path, logger=None, score_threshold=0,
    ...
    with torch.set_grad_enabled(False):
        for dt in tqdm(loader, disable=opt.disable_tqdm):
            ...
            output, loss = model(dt, criterion, opt.transformer_input_type, memory_bank, eval_mode=True)
